...
namespace {
    ...
    int getSocket(const std::string& host, int port, struct addrinfo*& res, struct addrinfo*& res0) noexcept {
        if(!socketFd) {
            ...
            for (res = res0; res; res = res->ai_next) {
                ...
                if((sockfd = socket(res->ai_family, res->ai_socktype | SOCK_NONBLOCK, res->ai_protocol)) < 0 ) {
                    continue;
                }
                ...
                case 0:
                    break;
                case EAI_SYSTEM:
                    debug("ERROR: UDP::sendMessage2Server getnameinfo");
                    break;
                default:
                    debug("ERROR: UDP::sendMessage2Server getnameinfo: %s", gai_strerror(herr));
                }
